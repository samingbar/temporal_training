...
- Understand every file in `src/workflows/agents/multi_agent_demo`.
...
All paths are under `src/workflows/agents/multi_agent_demo/`.
...
```bash
uv run -m src.workflows.agents.multi_agent_demo.worker
```
...
```bash
uv run -m src.workflows.agents.multi_agent_demo.run
```
