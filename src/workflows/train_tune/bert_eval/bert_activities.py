...
class BertFineTuneActivities:
    ...
    async def fine_tune_bert(self, request: BertFineTuneRequest) -> BertFineTuneResult:
        ...
        # Best-effort: after training completes, detect the latest checkpoint
        # directory on disk and signal the parent workflow with a CheckpointInfo
        # payload so it can update its internal state. This keeps the workflow
        # deterministic while still surfacing non-deterministic checkpoint paths.
        try:
            output_dir = Path(f"./bert_runs/{request.run_id}")
            ...
            if latest_checkpoint is not None:
                ...
                checkpoint_info = CheckpointInfo(
                    epoch=request.config.num_epochs,
                    step=0,
                ...
                await handle.signal("update_checkpoint", checkpoint_info)
        except Exception:  # noqa: BLE001
            # Never fail the training activity solely because checkpoint signaling failed.
            activity.logger.exception(
                "Failed to signal latest checkpoint for run %s",
                request.run_id,
            )
...
class BertEvalActivities:
    ...
    def _evaluate_bert_model_sync(self, request: BertEvalRequest) -> BertEvalResult:
        ...
        # Load the fine-tuned model and tokenizer from the run directory.
        model_dir = f"./bert_runs/{request.run_id}"
        tokenizer = AutoTokenizer.from_pretrained(model_dir)
        model = AutoModelForSequenceClassification.from_pretrained(model_dir)
        model.to(device)
