...
class RedundantWorkflow:
    async def run(req: SweepRequest) -> list[BertEvalResult]:
        ...
        # --- build initial trial configs (same as your SweepWorkflow) ---
        trial_cfgs: list[CoordinatorWorkflowConfig] = []
        for i in range(req.num_trials):
            # Copy base config
            cfg = req.base.model_copy(deep=True)
            ...
            cfg.evaluation_config.run_id = run_id
            ...
            # Max Sequence Length
            initial_max_seq_length = rng.choice(req.space.max_seq_length)
            cfg.fine_tune_config.max_seq_length = initial_max_seq_length
            cfg.evaluation_config.max_seq_length = initial_max_seq_length

            # Number of Epochs
            initial_num_epochs = rng.choice(req.space.num_epochs)
            cfg.fine_tune_config.num_epochs = initial_num_epochs

            # Set log-uniform learning rate
            lo, hi = req.space.learning_rate
            u = rng.random()
            cfg.fine_tune_config.learning_rate = float(
                math.exp(math.log(lo) + u * (math.log(hi) - math.log(lo)))
            ...
            cfg.fine_tune_config.seed = req.seed + i
            trial_cfgs.append(cfg)

        # --------------------------------------------------
        # Set Up Ladder Stages
        # --------------------------------------------------
        ...
        for stage_idx, (epochs, max_train, keep_k) in enumerate(stages):
            ...
            # rank by accuracy (or whatever metric you choose)
            stage_results_sorted = sorted(stage_results, key=lambda r: r.accuracy, reverse=True)
            top = stage_results_sorted[:keep_k]
            top_ids = {r.run_id for r in top}
            ...
            # if only one remains, you can break early
            if len(survivors) <= 1:
                break

        # Return the last stage's results for whoever survived
        # (If you want: return a leaderboard accumulated across stages instead.)
        final_results: list[BertEvalResult] = await workflow.execute_child_workflow(
            CoordinatorWorkflow.run,
            CoordinatorWorkflowInput(configs=survivors),
            id=f"coordinator-{req.experiment_id}-final",
        )
