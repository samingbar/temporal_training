...
class CheckpointedBertTrainingWorkflow:
    ...
    async def run(self, config: BertFineTuneConfig) -> BertFineTuneResult:
        ...
        # coordinator can control how training/eval artifacts are named.
        if config.run_id:
        ...
        # latest known checkpoint path (if one has already been recorded). The
        # checkpoint path itself is populated via signals from activities or
        # external processes; the workflow never inspects the filesystem so it
